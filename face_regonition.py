    ...
    while True:
        ...
        for face_location, face_encoding in zip(face_locations, face_encodings):
            ...
            if metadata is not None:
                ...
                face_label = f"In the park {int(time_at_door.total_seconds())}s"
        ...
        for metadata in known_face_metadata:
            ...
            if datetime.now() - metadata["last_seen"] < timedelta(seconds=5) and metadata["seen_frames"] > 1:
        ...
        if number_of_recent_visitors > 0:
            number = str(count)
            cv2.putText(frame, "Visitors in the Park: " + number, (5, 18), cv2.FONT_HERSHEY_DUPLEX, 0.8, (255, 255, 255), 1)
        ...
        if len(face_locations) > 0 and number_of_faces_since_save > 10:
            save_known_faces()
            number_of_faces_since_save = 0
        else:
